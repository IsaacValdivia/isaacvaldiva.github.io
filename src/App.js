...
import { GiOcarina, GiProcessor, GiFrogPrince } from "react-icons/gi";
...
const METAL_ICON = <GiOcarina/>;
const ELECTRONIC_ICON = <GiOcarina />;
const ELECTRONIC_CALM = <GiOcarina />;
const ELECTRONIC_INTENSE = <GiOcarina />;
const ORCHESTRA_ICON = <GiOcarina />;
const ORCHESTRA_CALM_ICON = <GiOcarina />;
const ORCHESTRA_INTENSE_ICON = <GiOcarina />;
const JAZZ_ICON = <GiOcarina />;
const PERFORMANCE_ICON = <GiOcarina />;

const TOP_LEFT = <Node title="Metal" icon={METAL_ICON} />;
const TOP_CENTER = <Node title="Orchestral" icon={METAL_ICON} />;
const TOP_RIGHT = <Node title="Electronic" icon={METAL_ICON} />;
const MID_LEFT = <Node title="Jazz" icon={METAL_ICON} />;
const MID_RIGHT = <Node title="Chamber" icon={METAL_ICON} />;
const BOT_LEFT = <Node title="Metal" icon={METAL_ICON} disabled/>;
const BOT_CENTER = <Node title="Performance" icon={METAL_ICON}/>;
  ...
  const toggle = () => setOpen(!dropdownOpen);
   ...
   return (
     <ButtonDropdown isOpen={dropdownOpen} toggle={toggle}>
       <DropdownToggle caret>EN</DropdownToggle>
       <DropdownMenu>
...
const App = () => {
    ...
    return (
      <div>
        ...
        <div className="vh-100 d-flex flex-column justify-content-start mainView">
          <div className="pt-1 px-1 d-flex flex-row justify-content-between">
            <ButtonGroup size={useMediaQuery({ query: "(max-width: " + mediaBreakpoint + "px )" }) ? "sm" : "md"}>
              <Button>AUDIO</Button>
              <Button outline>PROGRAMMING</Button>
            </ButtonGroup>

            <LanguagePicker />
          </div>

          <Container fluid={true} className="mainCont h-100 d-flex flex-column justify-content-around">
            <Row className="">
